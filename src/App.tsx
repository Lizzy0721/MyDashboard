...
function App() {
  ...
  return (
    <div className="max-w-screen min-h-screen bg-straw-100 flex font-montserrat">
      ...
      <div className={`lg:blur-none ${(isSidebar || isProfile) ? 'blur-2xl' : 'blur-none'} bg-straw-100 w-full`}>
        <Navbar onclickMenu={toggleSidebar} onclicked={toggleProofile} profileImgRef={dummyUser.userImg}/>
        <Outlet />
      </div>
      <Profile name={dummyUser.name} title={dummyUser.title} href={dummyUser.userImg} isOpen={isProfile} closeProfile={()=>setProfile(false)}/>
    </div>
  )
}
