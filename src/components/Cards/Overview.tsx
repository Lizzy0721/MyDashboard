...
function Details({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <p className="flex items-baseline text-xl font-semibold gap-1">
        {total}
        <p className="text-sm font-medium opacity-60">{subs}</p>
      </p>
    </div>
  );
}
...
export default function Overview({
  ...
  return (
    <div
      className={`${colorVariants[color]} p-4 rounded-xl w-full h-56 grid content-between`}
    >
      <div className="flex justify-between items-center">
        <p className="font-semibold text-xl">{name}</p>
        <button className="rounded-full bg-straw-200 p-1">
          <MoveUpRight className="size-[1em] stroke-2" />
        </button>
      </div>
      <div>
